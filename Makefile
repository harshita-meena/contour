...
container: ## Build the Contour container image
	docker build \
		...
		$(DOCKER_BUILD_LABELS) \
		$(shell pwd) \
		--tag gcr.io/etsy-mlinfra-dev/projectcontour/contour:jaeger-hmeena
	kind load docker-image $(IMAGE):$(VERSION) --name contour
	kubectl delete -f /home/hmeena/contour/examples/contour/ || true ## will fail on delete of job that has completed
	sleep 5
# Replace YAML image with the newly generated one ..
	git checkout 28ddd28e /home/hmeena/contour/examples/contour/03-contour.yaml
	sed -i "s/contour:v1.21.0/contour:$(VERSION)/g" /home/hmeena/contour/examples/contour/03-contour.yaml
	kubectl apply -f /home/hmeena/contour/examples/contour/
	sleep 2
	kubectl apply -f /home/hmeena/contour/examples/example-workload/httpproxy/03-routes/httpproxy-multiple-paths.yaml
	sleep 10
	@./hack/get_logs.sh
...
gl:
	@curl 127.0.0.1:9001/config_dump > /home/hmeena/contour/config_dump.yaml
